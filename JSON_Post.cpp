...
(std::string key)
{
  std::cout << "key: " << All_Keys.size() << std::endl;
...
(Json::Value myv)
{
  ...
  if ((myv["id"]).isNull() != true)
    ...
    {
      std::cerr << "1No Post ID presented" << std::endl;
      exit(-2);
    }
  //std::cout << "before calling json_post_common" << std::endl;
  int rc = Json_Post_Common(myv, my_post_ptr);
  //std::cout << "after calling json_post_common" << std::endl;
  if (rc != 0)
    {
...
(Json::Value myv, Post * my_post_ptr)
{
  ...
  // "actions"--------------------------------------------------------
    ...
    if (((myv["reactions"]).isNull() != true) &&
        (myv["actions"]["count"]).asInt() > 0) // and check if action is null
    {
  ...
  if ( ((myv["comments"]).isNull() != true ) &&
        ((myv["comments"]["count"]).asInt() > 0) )
    {
        ...
        for (int i = 0; i < (myv["comments"]["count"]).asInt(); i++)
        {
            ...
            {
                JvTime *reaction_ctime_ptr = new JvTime( ((l_jv["created_time"]).asString()).c_str());
                Person *react_p_ptr = new Person( 
                        (l_jv["from"]["avatar_name"]).asString(), 
                        (l_jv["from"]["vsID"]).asString() );
                std::string id = (l_jv["id"]).asString();
                Message *message = new Message ((l_jv["message"]).asString());
                Comment *comment_ptr = new Comment (
                        (my_post_ptr->profile_id), 
                        (my_post_ptr->post_id),
                        (id),
                        (react_p_ptr),
                        (message),
                ...
                if( ((l_jv["reactions"]).isNull() != true) && 
                    ((l_jv["reactions"]["count"]).asInt() >0) )
                    {
                        ...
                        for(int j = 0; j < (l_jv["reactions"]["count"]).asInt(); j++)
                        {
                            Json::Value r_jv =l_jv["reactions"]["data"][j];
                            if( ((r_jv["avatar_name"]).isNull() != true) &&
                                ((r_jv["vsID"]).isNull() != true) &&
                                ((r_jv["created_time"]).isNull() != true) &&
                                ((r_jv["type"]).isNull() != true) )
                                {
                                    Person *react_p_ptr = new Person( 
                                        (r_jv["avatar_name"]).asString(), 
                                        (r_jv["vsID"]).asString() );
                                    JvTime *created_time = new JvTime( ((r_jv["created_time"]).asString()).c_str());
                                    Reaction * react_ptr = new Reaction ((r_jv["type"]).asString(),
                                        react_p_ptr, created_time);
                        ...
                        (((l_jv["tags"]["count"]).asInt()) > 0) )
                    {
                        ...
                        for (int j = 0; j < (l_jv["tags"]["count"]).asInt(); j++)
                        {
                            Json::Value r_jv =l_jv["tags"]["data"][j];
                            if( ((r_jv["index"]).isNull() != true) &&
                                ((r_jv["avatar_name"]).isNull() != true) &&
                                ((r_jv["vsID"]).isNull() != true) &&
                                ((r_jv["comment"]).isNull() != true) )
                            {
                                Tag * tag_ptr = new Tag ( (r_jv["index"]).asInt(),
                                    (r_jv["avatar_name"]).asString(),
                                    (r_jv["vsID"]).asString(),
                                    (r_jv["comment"]).asString());
...
if( ((myv["keys"]).isNull() != true) && ((myv["keys"]["count"]).asInt()>0) )
{
    ...
    for(int i = 0; i < (myv["keys"]["count"]).asInt(); i++)
    {
        ...
        if(l_jv.isNull() != true)
        {
            ...
            //std::pair<std::string, OKey *>(index, okey_ptr);
            //All_Keys.insert(std::pair<std::string, OKey *>(index, okey_ptr));
        }
    }
}
// std::map<std::string, OKey *> All_Keys;
  ...
  if ((myv["message"]).isNull() != true)
      {
          ...
          //std::cout << "MESSAGE SECTION" << std::endl;
